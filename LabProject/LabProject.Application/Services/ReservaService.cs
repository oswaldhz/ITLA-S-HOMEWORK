...
public class ReservaService : IReservaService
{
    ...
    public async Task<ReservaDto?> ActualizarAsync(int id, CreateReservaRequest request, ReservationValidationResult? validationResult = null, CancellationToken cancellationToken = default)
    {
        validationResult ??= await _reservationService.ValidateForUpdateAsync(id, request, cancellationToken);

        var updated = await _reservaRepository.UpdateAsync(
            id,
            validationResult.Usuario.Id,
            validationResult.Equipo.Id,
            request.FechaInicio,
            request.FechaFin,
            request.SoftwareIds,
            cancellationToken);
        ...
        return Map(updated, validationResult.Usuario, validationResult.Equipo);
    }
    ...
    private static ReservaDto Map(Reserva reserva, Usuario usuario, Equipo equipo)
    {
        ...
        // El frontend envía fechas como ISO en UTC (Date.toISOString()).
        // EF Core materializa DateTime como Kind=Unspecified al leer desde la BD.
        // Si devolvemos DateTime sin Kind=Utc, el JSON sale sin "Z" y el navegador
        // lo interpreta como hora local, produciendo desfases en la Agenda.
        ...
        return new ReservaDto(
            ...
            AsUtc(reserva.FechaInicio),
            AsUtc(reserva.FechaFin),
            softwareIds);
    }
}
